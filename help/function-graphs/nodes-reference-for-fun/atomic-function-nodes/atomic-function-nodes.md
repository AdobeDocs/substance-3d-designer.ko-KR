...
|                                                                                                                                        | 벡터 Float3 | 부동 1 / 부동 2 | 부동 3 | 3개 좌표의 벡터에 2개의 부동 값을 캐스팅합니다. |
|                                                                                                                                        | 벡터 부동 소수점4 | Float1 / 2 / 3 | 부동 4 | 4개 좌표의 벡터에 2개의 부동 값을 캐스팅합니다. |
|                                                                                                                                        | 스위즐 부동 소수점1 | 벡터 부동 소수점 | Float1 | 벡터에서 부동 좌표 추출 |
|                                                                                                                                        | 스위즐 부동 소수점2 | 벡터 부동 소수점 | 실수2 | 벡터에서 2개의 부동 좌표를 추출합니다. |
|                                                                                                                                        | 스위즐 부동 소수점3 | 벡터 부동 소수점 | 실수3 | 벡터에서 3개의 부동 좌표를 추출합니다. |
|                                                                                                                                        | 스위즐 부동 소수점4 | 벡터 부동 소수점 | 실수4 | 벡터에서 4개의 부동 좌표를 추출합니다. |
...
|                                                                                                                                        | 부동 받기 1 | - | Float1 | 함수 또는 그래프의 부동 값 입력 가져오기 |
|                                                                                                                                        | 부동 소수점2 얻기 | - | 실수2 | 함수 또는 그래프 부동2 값 입력 가져오기 |
|                                                                                                                                        | 부동 소수점3 얻기 | - | 실수3 | 함수 또는 그래프 부동3 값 입력 가져오기 |
|                                                                                                                                        | 부동 소수점4 얻기 | - | 실수4 | 함수 또는 그래프 부동4 값 입력 가져오기 |
...
|                                                                                                                                        | 정수로 | Float1 | Integer1 | 정수로 부동을 변환합니다. |
|                                                                                                                                        | 정수2로 | 실수2 | 정수2 | Integer2의 부동2를 변환합니다 |
|                                                                                                                                        | 정수3으로 | 실수3 | 정수3 | Integer3의 부동3을 변환합니다 |
|                                                                                                                                        | 정수4로 | 실수4 | 정수4 | Integer4의 부동4를 변환합니다 |
| [연산자](../../../function-graphs/nodes-reference-for-fun/atomic-function-nodes/operator-nodes/operator-nodes.md) | 추가 | 벡터 부동 소수점 / 정수 | a &amp; b 유형 | 동일한 유형의 두 값(a + b)을 추가합니다. |
|                                                                                                                                        | 뺄셈 | 벡터 부동 소수점 / 정수 | a &amp; b 유형 | 동일한 유형의 두 값(a - b)을 뺍니다 |
|                                                                                                                                        | 곱셈 | 벡터 부동 소수점 / 정수 | a &amp; b 유형 | 동일한 유형의 두 값(a \* b)을 곱합니다. |
|                                                                                                                                        | 스칼라 곱셈 | 벡터 부동 소수점 | 문자 유형 | 값에 부동 값을 곱합니다. \* 스칼라 |
|                                                                                                                                        | 나눗셈 | 부동1 / Integer1 | a &amp; b 유형 | 동일한 유형의 두 값 a/b를 나눕니다. |
|                                                                                                                                        | 부정 | 부동1 / Integer1 | 문자 유형 | 부정 값을 반환합니다. -a |
|                                                                                                                                        | 모듈로 | 부동1 / Integer1 | 문자 유형 | 모듈로 값을 반환합니다. mod(a, diger) |
|                                                                                                                                        | 도트 곱 | 벡터 부동 소수점 | a &amp; b 유형 | dot(a, b)와 같이 동일한 유형의 2개 값의 내적을 반환합니다. |
...
| [비교](../../../function-graphs/nodes-reference-for-fun/atomic-function-nodes/comparison-nodes/comparison-nodes.md) | Equal | 부동1 / Integer1 | 부울 | a = b인 경우 true를 반환합니다. |
|                                                                                                                                        | 같지 않음 | 부동1 / Integer1 | 부울 | != b인 경우 true를 반환합니다. |
|                                                                                                                                        | 보다 큼 | 부동1 / Integer1 | 부울 | a > b인 경우 true를 반환합니다. |
|                                                                                                                                        | 크거나 같음 | 부동1 / Integer1 | 부울 | a >= b이면 true를 반환합니다. |
|                                                                                                                                        | 보다 작음 | 부동1 / Integer1 | 부울 | &lt; b인 경우 true를 반환합니다. |
|                                                                                                                                        | 작거나 같음 | 부동1 / Integer1 | 부울 | a &lt;= b이면 true를 반환합니다. |
| 함수 | 절대치 | 부동1 / Integer1 | Float1 | a의 절대값을 구합니다. abs(a) |
|                                                                                                                                        | 내림 | 부동1 / Integer1 | Float1 | -floor(a)보다 작거나 같은 가장 큰 값을 반환합니다. |
|                                                                                                                                        | 상한 | 부동1 / Integer1 | Float1 | 다음보다 크거나 같은 가장 작은 값을 구합니다. ceil(a) |
|                                                                                                                                        | 코사인 | 부동1 / Integer1 | Float1 | a의 코사인 값을 구합니다. cos(a) |
|                                                                                                                                        | 사인 | 부동1 / Integer1 | Float1 | a의 사인 값을 반환합니다. sin(a) |
|                                                                                                                                        | 탄젠트 | 부동1 / Integer1 | Float1 | tan(a)의 탄젠트 값을 반환합니다. |
|                                                                                                                                        | 아크 접선 2 | 벡터 부동 소수점2 | Float1 | vector2 항목의 arc tan 2 값을 반환합니다. arctan2(xa, ya) |
|                                                                                                                                        | 데카르트식 | Float1 | 실수2 | 2극좌표를 카트 좌표(rho, theta)로 변환합니다. |
|                                                                                                                                        | 제곱근 | 부동1 / Integer1 | Float1 | 의 제곱근 값을 반환합니다. |
|                                                                                                                                        | 로그 값 | 부동1 / Integer1 | Float1 | 로그 값 a: log(a)를 반환합니다. |
|                                                                                                                                        | 지수 | 부동1 / Integer1 | Float1 | 지수 값 a를 구합니다. exp(a) |
|                                                                                                                                        | 펑 2 | 부동1 / Integer1 | Float1 | a의 2제곱값을 구합니다. |
|                                                                                                                                        | 선형 보간 | 부동1 / Integer1 | Float1 | 부동 값에 따라 두 값 사이의 선형 보간을 반환합니다. (1-x)a + x \* b |
|                                                                                                                                        | 최소 | 부동1 / Integer1 | a &amp; b 유형 | a와 b 사이의 최소값을 반환합니다. |
|                                                                                                                                        | 최대 | 부동1 / Integer1 | a &amp; b 유형 | a에서 b 사이의 최대값을 반환합니다. |
