...
Substance 3D Designer은 노드 기반의 편집기입니다. 즉, 거의 모든 유형의 프로젝트 또는 자원에는 노드(빌딩 블록)를 배치하고 이들을 연결하여 작업 체인(그래프)을 만드는 작업이 포함됩니다.\
이 페이지에서는 노드 기반 워크플로우의 개념에 대해 설명하고 Designer에서 작성할 수 있는 3가지 주요 그래프 유형에 대한 요약을 제공합니다.

![데이터 흐름이 단순화됨](workflow-overview.resources/graph-direction.png "데이터 흐름이 단순화됨"){zoomable="yes"}
...
그래프 인스턴스 지정은 Designer의 주요 프로세스입니다. 이를 통해 그래프나 그래프의 일부를 재사용 가능한 노드로 패키징하여 고유한 노드를 구축할 수 있습니다. 이러한 노드를 [인스턴스 노드](../../compositing-graphs/creating-compositing-gra/graph-instances-sub-gra/graph-instances-sub-graphs.md)라고 하며 그래프를 다시 사용하여 보다 효율적으로 작업할 수 있습니다.\
예: 가장자리 마모를 위한 훌륭한 기술을 개발했습니까? 별도의 그래프로 분리하고 다른 프로젝트에서 재사용합니다!

그래프 인스턴스에 대한 자세한 내용은 [Substance 그래프](../../compositing-graphs/substance-compositing-graphs.md)에서 사용하는 [전용 섹션](../../compositing-graphs/creating-compositing-gra/graph-instances-sub-gra/graph-instances-sub-graphs.md)을 참조하세요.

![그래프 매개 변수 단순화](workflow-overview.resources/parameters-5.png "그래프 매개 변수 단순화"){zoomable="yes"}

## 사용자 정의 매개 변수

작업 체인의 모든 노드에는 최종 결과에 영향을 미치는 버튼, 슬라이더, 설정을 조정할 수 있는 몇 가지 형태의 컨트롤이 있습니다.\
하위 그래프를 만들거나 Substance 파일을 다른 애플리케이션으로 내보내려는 경우, 다른 사용자가 완전히 고유한 제어판으로 그래프를 조정하고 수정할 수 있도록 자신의 &quot;제어판&quot;을 만들 수 있습니다.

사용자 지정 매개 변수 [여기](../../compositing-graphs/compositing-graph-key-con/substance-compositing-graph-key-concepts.md)의 일반적인 개념에 대해 알아보거나, 깊이에서 더 나아가 [매개 변수 노출을 시작](../../compositing-graphs/manage-parameters/exposing-a-parameter/exposing-a-parameter.md)하십시오.
...
![Substance 그래프의 단순 모양](workflow-overview.resources/simpleshape.png "Substance 그래프의 단순 모양"){width="512px" zoomable="yes"}
...
![Substance 그래프의 전체 재질](workflow-overview.resources/simplematerial.png "Substance 그래프의 전체 재질"){width="512px" zoomable="yes"}

더 복잡한 그래프 [두 기본 재질을 혼합](../../compositing-graphs/nodes-reference-for-com/node-library/material-filters/blending-material/material-blend/material-blend.md)합니다. [기본 재질](../../compositing-graphs/nodes-reference-for-com/node-library/material-filters/pbr-utilities/base-material/base-material.md) 중 하나는 간단하게 유지되고 다른 하나는 일부 사용자 지정 입력을 사용하여 관심을 추가합니다. 마스크는 최종 [출력](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/output/output.md)(으)로 설정되기 전에 두 재질 중 표시할 재질을 결정하는 데 사용됩니다.
...
실제로 워크플로우는 **수학적 및 논리적 연산**&#x200B;을 기반으로 하므로 Designer에서 작업할 때 훨씬 더 진보된 방법입니다.

함수는 다음과 같은 다양한 컨텍스트에서 사용할 수 있습니다.
...
![단순 함수 그래프](workflow-overview.resources/lerpfunction.png "단순 함수 그래프"){width="256px" zoomable="yes"}
...
이 함수는 HLSL 또는 GLSL과 유사한 의사 코드로 `lerp(0.1, 0.8, Intensity)`으로 기록될 수도 있습니다.

+++

+++ 고급 기능

![고급 함수](workflow-overview.resources/pixel-function.png "고급 함수"){width="512px" zoomable="yes"}
...
의사 코드에서는 한 줄에 맞지 않는 훨씬 더 복잡한 함수가 될 것이다.
