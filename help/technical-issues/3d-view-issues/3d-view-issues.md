...
새로운 3D 렌더러(래스터라이저 및 GPU 패스트레이서)를 사용할 경우 높은 폴리 메시에서 변위를 사용할 때 3D 보기의 성능이 저하될 수 있습니다.

![(틱)](3d-view-issues.resources/check.svg) **권장 단계**

3D 뷰에서 사용할 수 있는 테셀레이션 기술은 메시를 세분화하여 변위 사용 시 더 많은 세부 사항을 제공하는 기술로, 실제로 적용되는 테셀레이션에 관계없이 실행되는 준비 단계를 포함합니다.
이 섹션의 [팁](../../interface/3d-view/displacement/displacement.md#per-mesh)은 장면에서 `Mesh` 개체당 쪽맞춤 프로세스를 비활성화하여 성능 문제를 해결합니다.
