...
breadcrumb-title: ''
description: 보로노이 노드를 사용하여 셀룰러 텍스처와 유기적인 재료 효과를 만들기 위한 보로노이 패턴을 생성합니다.
...
* **크기** *부동 소수점3*\
  **X**, **Y** 및 **Z** 축의 보로노이 노이즈 크기를 제어합니다. 균일하지 않은 값을 사용하면 *스트레치 또는 스쿼싱* 효과가 발생합니다.\
  *참고*: *모든 축*&#x200B;에서 **타일링**&#x200B;을 사용할 수 있는 경우 크기 조정은 *단계*&#x200B;입니다. 이것은 예상된 일입니다.
* **오프셋** *부동 소수점3*\
  **X**, **Y** 및 **Z** 축에서 보로노이 노이즈의 *위치*&#x200B;에 오프셋을 적용합니다.
* **장애** *부동 소수점3*\
  **X**, **Y** 및 **Z** 축의 각 노이즈 지점에 적용된 *임의 오프셋*&#x200B;의 강도입니다.
* **왜곡 강도** *부동*\
  보로노이 노이즈에 적용되는 *뒤틀기 효과*&#x200B;의 강도를 제어합니다.
* **왜곡 배율 배율** *부동*\
...
* **스타일** *정수*&#x200B;잡음이 공간의 점 집합을 기반으로 한다는 점을 고려하여 보로노이 잡음의 *데이터를 렌더링하는* 방법을 설정합니다.
...
* **가장자리 Thickness** *부동*&#x200B;보로노이 잡음의 셀 사이에서 감지된 가장자리의 Thickness을 조정합니다. 가장자리는 X, Y 및 Z축에서 감지되므로 셀의 *깊이*&#x200B;에 따라 일부 두께가 다른 두께보다 빠르게 증가할 수 있습니다.\
