---
helpx_url: "https://helpx.adobe.com/kr/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs.html"
breadcrumb-title: ''
description: Designer의 Substance 합성 그래프에 대한 전체 노드 참조에 액세스하여 사용 가능한 모든 노드에 대해 알아보십시오.
helpx_creative_field: ""
helpx_description: Designer > Substance graphs > Nodes reference for Substance graphs
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Substance 그래프의 노드 참조
user-guide-description: ''
user-guide-title: ''
source-git-commit: 4f8830fa9ab6012f0a7ba5054eb171b151c44874
workflow-type: tm+mt
source-wordcount: '121'
ht-degree: 16%

---


# Substance 그래프의 노드 참조

Substance 3D Designer은 노드 기반의 텍스처링 소프트웨어이므로 다양한 노드를 자유롭게 사용할 수 있습니다. 본 절에서는 소프트웨어와 함께 제공되는 모든 노드의 종류와 문서에 대해 설명합니다.

SD 내에서 &quot;노드&quot;라고 부르는 것은 두 개의 주요 그룹으로 나눌 수 있습니다.

* 모든 그래프의 핵심 구성 요소인 [원자 노드](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/atomic-nodes.md)입니다.
* [그래프 인스턴스](../../compositing-graphs/creating-compositing-gra/graph-instances-sub-gra/graph-instances-sub-graphs.md), 복합 노드입니다. 이 중 많은 사전 제작 콘텐츠를 [노드 라이브러리](../../compositing-graphs/nodes-reference-for-com/node-library/node-library.md)에서 사용할 수 있습니다.

## 이 섹션에서:

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

[노드 라이브러리](../../compositing-graphs/nodes-reference-for-com/node-library/node-library.md)

* [텍스처 생성기](node-library/texture-generators/texture-generators.md)
* [필터](node-library/filters/filters.md)
* [재질 필터](node-library/material-filters/material-filters.md)
* [메시 기반 생성기](node-library/mesh-based-generators/mesh-based-generators.md)
* [스플라인 및 패스 도구](node-library/spline-paths-tools/spline-paths-tools.md)

</td>
<td style="border: 0;" valign="top">

[원자성 노드](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/atomic-nodes.md)

</td>
</tr>
</table>
